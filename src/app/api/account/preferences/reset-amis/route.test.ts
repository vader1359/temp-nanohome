...
describe("/api/account/preferences/reset-amis", () => {
  ...
  it("reports reauthentication without exposing AMIS data", async () => {
    ...
    const response = await POST(sameOriginRequest("https://app.test/api/account/preferences/reset-amis", { method: "POST" }));
  ...
  it("rejects a non-JSON body before resetting AMIS history", async () => {
    ...
    const response = await POST(sameOriginRequest("https://app.test/api/account/preferences/reset-amis", {
      body: "unexpected",
