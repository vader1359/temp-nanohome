...
function CartButton({ label, variant, onClick, count }: { label: string; variant: "home" | "catalog"; onClick: () => void; count: number }) {
  return (
    <button
      ...
      type="button"
      onClick={onClick}
      className={variant === "catalog" ? "text-nh-ink transition-opacity hover:opacity-70" : "relative flex items-center lg:hidden"}
    >
        ...
        <div className="site-shell flex flex-col items-center gap-4">
          <div className="hidden w-full items-center justify-between gap-8 lg:flex">
            ...
            <nav className="flex items-center gap-5" aria-label="Utility navigation">
              {topRight.map((item) => (
                <HeaderLinkItem key={item.key} item={item} className="flex items-center gap-1 text-[12px] font-normal leading-[18px] text-nh-muted" />
              ))}
            </nav>
          </div>
    ...
    <header className="relative z-30 min-h-[80px] bg-white lg:h-[150px]">
      <div className="site-shell py-4">
        <div className="hidden items-center justify-between border-b border-[#cfc9c0] pb-4 lg:flex">
          ...
          <div className="flex gap-5">
            {topRight.map((item) => (
              <HeaderLinkItem key={item.key} item={item} className="flex items-center gap-1 text-xs leading-[18px] text-[#666]" />
            ))}
          </div>
        </div>
        <div className="flex items-center justify-between gap-4 pt-2 lg:h-[83px] lg:gap-0 lg:pt-0">
          ...
          <DesktopIcons locale={locale} iconLabels={iconLabels} variant={variant} accountIcon={AccountIcon} onOpenCart={openCart} cartCount={cartCount} />
        </div>
        <HeaderDrawer
          drawerOpen={drawerOpen}
          onCloseDrawer={onCloseDrawer}
...
function HeaderMainRow({
  ...
  variant,
  accountIcon: AccountIcon,
}: Pick<SharedHeaderProps, "drawerOpen" | "onToggleDrawer" | "logo" | "locale" | "iconLabels" | "menuLabel" | "closeLabel" | "variant" | "accountIcon"> & {
  accountIcon: ComponentType<{ className?: string }>;
  onOpenCart: () => void;
  cartCount: number;
}) {
  ...
  return (
    <>
      <button
        ...
        className={isCatalog ? "flex size-8 shrink-0 items-center justify-center text-nh-ink lg:hidden" : "flex size-8 shrink-0 items-center justify-center text-[#111] lg:hidden"}
      ...
      {isCatalog ? (
        <div className="flex min-w-0 items-center gap-3 sm:gap-5">
          <button className="hidden text-nh-ink transition-opacity hover:opacity-70 sm:block" type="button" aria-label={iconLabels.search}>
            <Search className="size-5" />
          </button>
          <button className="hidden text-nh-ink transition-opacity hover:opacity-70 sm:block" type="button" aria-label={iconLabels.wishlist}>
            <Heart className="size-5" />
          </button>
          <div className="relative">
            <CartButton label={iconLabels.cart} variant="catalog" onClick={onOpenCart} count={cartCount} />
          </div>
          <button className="hidden text-nh-ink transition-opacity hover:opacity-70 sm:block" type="button" aria-label={iconLabels.account}>
...
function DesktopIcons({
  ...
  return (
    <div className="hidden lg:flex lg:items-center lg:gap-5 lg:ml-auto">
      <button type="button" aria-label={iconLabels.search}>
        <Search className="size-5 stroke-[1.4]" />
      </button>
      <button type="button" aria-label={iconLabels.wishlist}>
        <Heart className="size-5 stroke-[1.4]" />
      </button>
      <button type="button" onClick={onOpenCart} aria-label={iconLabels.cart} className="relative">
        <ShoppingCart className="size-5 stroke-[1.4]" />
        <span className="absolute -bottom-1 -left-1 grid size-3 place-items-center rounded-full bg-[#930000] text-[8px] text-white">{cartCount}</span>
      </button>
      <button type="button" aria-label={iconLabels.account}>
        <AccountIcon className="size-5 stroke-[1.4]" />
      </button>
      <LocaleToggle locale={locale} placement="desktop" />
    </div>
  );
      ...
      <div className={cn("border-t pt-4 pb-2", borderClassName)}>
        ...
        {isCatalog ? (
          <div className="flex flex-col gap-3 border-b border-nh-border py-4 sm:pt-0">
            {topLeft.map((item) => (
              <HeaderLinkItem key={item.key} item={item} className="text-[13px] font-normal leading-[18px] text-nh-muted" />
            ))}
          </div>
        ) : null}
        <nav className={cn("flex flex-col gap-3 border-b py-4", borderClassName)} aria-label={isCatalog ? "Categories" : undefined}>
          {nav.map((item) => (
            <HeaderLinkItem key={item.key} item={item} onClick={onCloseDrawer} className={isCatalog ? "text-sm font-normal uppercase leading-5 text-nh-ink" : "text-sm font-normal uppercase leading-5 text-[#111]"} />
          ))}
        </nav>
        <div className="flex flex-col gap-3 pt-4">
          {isCatalog
            ? null
            : topLeft.map((item) => <HeaderLinkItem key={item.key} item={item} onClick={onCloseDrawer} className="text-sm leading-[18px] text-[#666]" />)}
          {topRight.map((item) => (
            <HeaderLinkItem key={item.key} item={item} onClick={onCloseDrawer} className={cn("flex items-center gap-1 leading-[18px]", isCatalog ? "text-[13px] text-nh-muted" : "text-sm text-[#666]")} />
          ))}
        </div>
      </div>
    </div>
