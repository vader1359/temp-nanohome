...
function LocaleToggle({
  ...
  const wrapperClassName =
    placement === "desktop" ? locale.desktopClassName : locale.mobileClassName;

  const localeItem = (code: "VN" | "EN") => {
    const className = cn(
      current === code.toLowerCase() ? active : inactive,
      locale.onChange && current !== code.toLowerCase()
        ? "hover:text-nh-ink"
        : undefined,
    );
...
function CartButton({
  ...
  return (
    <button
      ...
      className={
        variant === "catalog"
          ? "text-nh-ink transition-opacity hover:opacity-70"
          : "relative flex items-center lg:hidden"
      }
    >
      <ShoppingCart
        className={variant === "catalog" ? "size-5" : "size-5 stroke-[1.4]"}
      />
      <span
        className={
          variant === "catalog"
        ...
        aria-label={
          variant === "catalog" ? "Số lượng trong giỏ hàng" : undefined
        }
      >
        1
      </span>
            ...
            >
              {topRight.map((item) => (
                <HeaderLinkItem
                  key={item.key}
                  item={item}
                  className="flex items-center gap-1 text-[12px] font-normal leading-[18px] text-nh-muted"
                />
              ))}
            </nav>
          </div>
  ...
  return (
    <>
      <button
        ...
        className={
          isCatalog
            ? "flex size-8 shrink-0 items-center justify-center text-nh-ink lg:hidden"
            : "flex size-8 shrink-0 items-center justify-center text-[#111] lg:hidden"
        }
      ...
      {isCatalog ? (
        <div className="flex min-w-0 items-center gap-3 sm:gap-5">
          ...
          <button
            className="hidden text-nh-ink transition-opacity hover:opacity-70 sm:block"
            type="button"
            aria-label={iconLabels.wishlist}
          >
            <Heart className="size-5" />
          </button>
          <div className="relative">
            <CartButton label={iconLabels.cart} variant="catalog" />
          </div>
          <button
            className="hidden text-nh-ink transition-opacity hover:opacity-70 sm:block"
            type="button"
...
function DesktopIcons({
  ...
  return (
    <div className="hidden lg:flex lg:items-center lg:gap-5 lg:ml-auto">
      ...
      <button aria-label={iconLabels.wishlist}>
        <Heart className="size-5 stroke-[1.4]" />
      </button>
      <button aria-label={iconLabels.cart} className="relative">
      ...
      <button aria-label={iconLabels.account}>
  ...
  return (
    ...
    >
      <div className={cn("border-t pt-4 pb-2", borderClassName)}>
        ...
        <nav
          className={cn("flex flex-col gap-3 border-b py-4", borderClassName)}
          aria-label={isCatalog ? "Categories" : undefined}
        >
          {nav.map((item) => (
            <HeaderLinkItem
              key={item.key}
        ...
        <div className="flex flex-col gap-3 pt-4">
          ...
          {topRight.map((item) => (
            <HeaderLinkItem
              ...
              onClick={onCloseDrawer}
              className={cn(
                "flex items-center gap-1 leading-[18px]",
                isCatalog ? "text-[13px] text-nh-muted" : "text-sm text-[#666]",
              )}
            />
          ))}
        </div>
