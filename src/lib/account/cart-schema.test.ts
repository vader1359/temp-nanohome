...
describe("cart schemas", () => {
  it("rejects presentation and account fields in a cart mutation", () => {
    ...
    const body = { expectedVersion: 0, price: 1, quantity: 1, title: "forged", variantId };
  ...
  it("accepts only bounded guest id and quantity pairs", () => {
    ...
    const body = { idempotencyKey: "merge-001", items: [{ quantity: 2, variantId }] };
  ...
  it("rejects arbitrary product text instead of a canonical variant UUID", () => {
    expect(parseCartMutation({
      expectedVersion: 0,
      quantity: 1,
      variantId: "chair-oak",
    })).toBeNull();
