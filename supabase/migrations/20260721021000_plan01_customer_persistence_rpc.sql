...
declare v_name text := p_event->>'name'; v_key text := encode(digest(coalesce(p_event->>'idempotencyKey', gen_random_uuid()::text), 'sha256'), 'hex'); v_properties jsonb := p_event->'properties';
begin
