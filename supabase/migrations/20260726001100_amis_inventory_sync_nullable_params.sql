-- Re-declare apply_amis_inventory_sync with DEFAULT NULL on the three parameters
-- that are legitimately nullable: p_watermark, p_expected_baseline_id, and
-- p_expected_watermark.  The SQL body is unchanged; adding defaults lets the
-- Supabase type-generator emit them as optional (?: string) so TypeScript
-- callers are not forced to cast null to string.
create or replace function public.apply_amis_inventory_sync(
  ...
  p_expected_watermark timestamptz default null
) returns jsonb
language plpgsql
security definer
