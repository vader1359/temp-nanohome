...
create index if not exists variants_fuzzy_name_vi_trgm_idx
  on public.variants
  using gin (name_vi gin_trgm_ops);

create index if not exists variants_fuzzy_name_trgm_idx
  on public.variants
  using gin (name gin_trgm_ops);

create index if not exists variants_fuzzy_sku_trgm_idx
  on public.variants
  using gin (sku gin_trgm_ops);

create index if not exists variants_fuzzy_finish_vi_trgm_idx
  on public.variants
  using gin (finish_vi gin_trgm_ops);

create index if not exists variants_fuzzy_finish_trgm_idx
  on public.variants
  using gin (finish gin_trgm_ops);

create index if not exists variants_fuzzy_brand_name_denorm_trgm_idx
  on public.variants
  using gin (brand_name_denorm gin_trgm_ops);
