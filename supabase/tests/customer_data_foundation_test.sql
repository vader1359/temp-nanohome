...
select lives_ok($$ select public.append_customer_event('00000000-0000-4000-8000-000000000201', '00000000-0000-4000-8000-000000000202', '{"name":"page_viewed","idempotencyKey":"rpc-page-00000001","properties":{"routeKey":"/","locale":"vi"}}'::jsonb, now()) $$, 'event RPC accepts fixed columns with granted analytics');
select throws_ok($$ select public.append_customer_event('00000000-0000-4000-8000-000000000201', '00000000-0000-4000-8000-000000000202', '{"name":"recommendation_impression","idempotencyKey":"rpc-rec-000000001","properties":{"requestId":"r","placement":"home","itemIds":["00000000-0000-4000-8000-000000000031"]}}'::jsonb, now()) $$, 'P0001', 'required consent purpose is not granted', 'event RPC requires mapped personalization consent');
select throws_ok($$ select public.append_customer_event('00000000-0000-4000-8000-000000000201', '00000000-0000-4000-8000-000000000202', '{"name":"page_viewed","idempotencyKey":"rpc-raw-000000001","properties":{"routeKey":"/","locale":"vi","rawPayload":{}}}'::jsonb, now()) $$, 'P0001', 'unknown event properties are not allowed', 'event RPC rejects unknown properties');
select is((select count(*) from public.customer_events where idempotency_key_hash = encode(extensions.digest('rpc-page-00000001', 'sha256'), 'hex')), 1::bigint, 'event RPC stores one fixed-column record without payload');
